...
class RepetitionGuardTest(unittest.TestCase):
    def test_strips_repeated_emoji_from_text_without_dropping_words(self):
    ...
    def test_drops_repeated_meme_item_and_keeps_text(self):
        decision = ActionDecision(
            action=Action.REACT,
            items=[
    ...
    def test_duplicate_reaction_only_turn_becomes_wait(self):
        ...
        result = filter_recent_repeated_reactions(decision, ["[表情: meme:helpless_facepalm]"])
    ...
    def test_repetition_reminder_is_internal_system_message(self):
        ...
        result = filter_recent_repeated_reactions(decision, ["刚刚也笑了😂"])
        ...
        self.assertTrue(any("不要继续使用" in rule for rule in payload["rules"]))
